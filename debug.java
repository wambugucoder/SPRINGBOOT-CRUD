...
//        queryString.append("VATDESCRIPTION "); //46
        ...
        // START JIRA GRIM-822
        queryString.append("COMMISSION_VAT_AMOUNT, ISDELETEDELEMENT, ISCHANGEDPRICE, ISPRICEDECREASED, IS_REBATE, IS_SURCHARGE, FLEX_LEVEL, ");
        // END
        manageAdditionalFieldsForAggregation(queryString, false); //CFL-20
...
//        start = new Date();
        ...
        Collections.sort(queryResult, new QueryResultsComparator());
...
//        logger.fine("********************* PERFORMANCE TEST generateSingleInvoice ---> fillDataCacheProductMap: " + (DateUtils.getElapsedTime(start, new Date())));
        ...
        for (Object rowObj : queryResult) {
            ...
            //                      - CREDIT NOTE
            //                      - INVOICE and ISBULKAMENDMENT
            BigDecimal productDiscountAmount = (BigDecimal) row.get(12);
...
//        start = new Date();
        ...
        sb = new StringBuilder();
        sb.append("SELECT DISTINCT IBF.PRICELISTFEENAME,");
        sb.append("       IBF.DESCRIPTION,");
        sb.append("       DL.NETAMOUNT,");
        sb.append("       DL.COMMISSIONAMOUNT,");
        sb.append("       NVL (");
        sb.append("          (SELECT SUM (DLFV.VATAMOUNT)");
        ...
        sb.append("                  AND DLFV.VATON = 'FEE'),");
        sb.append("          0)");
        sb.append("          AS vatamount, "); //DL.VATAMOUNT,    //BCFER-87 BCF GAP CTR 006 - Company Tax Regulation
        ...
        sb.append("                  FROM oa_documentlinefeevat v, oa_documentline docline");
        ...
        sb.append("           AND vat_query.linenumber = dl.linenumber) vatcode,"); // DL.VATCODE,   //BCFER-87 BCF GAP CTR 006 - Company Tax Regulation
        ...
        sb.append("                  AND dlfv.DOCUMENTID = dl.DOCUMENTID");
        sb.append("                  AND dlfv.DOCUMENTSTARTDATE = dl.DOCUMENTSTARTDATE");
        sb.append("                  AND DLFV.VATON = 'COMMISSION'),");
        sb.append("          0)");
        sb.append("          AS commissionvatamount, "); //IBF.COMMISSIONVATAMOUNT,   //BCFER-87 BCF GAP CTR 006 - Company Tax Regulation
        sb.append("       '' commissionvatcode, "); //IBF.COMMISSIONVATCODE,    //BCFER-87 BCF GAP CTR 006 - Company Tax Regulation
        ...
        sb.append("                  FROM oa_documentlinefeevat v, oa_documentline docline, oa_vatrule vr, oa_invoicestatement ist  ");
        sb.append("                 WHERE     v.LINENUMBER = docline.LINENUMBER" );
        sb.append("                   AND v.LINESTARTDATE = docline.STARTDATE");
        sb.append("                   AND v.DOCUMENTID = docline.DOCUMENTID");
        sb.append("                   AND v.DOCUMENTSTARTDATE = docline.DOCUMENTSTARTDATE");
        ...
        sb.append("                   AND ist.VATRULESTARTDATE = vr.STARDATE " );
        sb.append("               ) vat_query");
        sb.append("         WHERE vat_query.documentid = DL.DOCUMENTID");
        sb.append("           AND vat_query.linenumber = dl.linenumber) vatdescription ");
        //END CFL-2329
        sb.append("  FROM OA_DOCUMENTLINE DL, OA_DOCLINEBOOKINGFEE DLBF, IBE_BOOKINGFEE IBF");
        sb.append(" WHERE     DL.DOCUMENTID = ? ");
        sb.append("       AND DL.ENDDATE = '99991231000000'");
        sb.append("       AND DLBF.DOCUMENTLINENUMBER = DL.LINENUMBER");
        ...
        sb.append("       AND DLBF.BOOKINGVERSION = IBF.BOOKINGVERSION");
        ...
        sb.append("       AND DLBF.BOOKINGFEESTARTDATE = IBF.STARTDATE ");// </editor-fold>
        if (isBulkAttachment) {
        ...
        for (InvoiceRouteDto invoiceRouteDto : invoiceRouteList) {
            ...
            for (InvoiceRowDto invoiceRowDto : invoiceRowList) {
                ...
                for (InvoiceProductDto invoiceProductDto : subProductList) {
                    ...
                    bookingVatTotal = MathUtil.sum(bookingVatTotal, productVat);
                    ...
                    totalRouteVat = MathUtil.sum(totalRouteVat, productVat);
                    totalRouteVatCommission = MathUtil.sum(totalRouteVatCommission, productCommissionVat);
                    totalRouteAmount = MathUtil.sum(totalRouteAmount, productSubTotal, productVat);
                }
            }
            ...
            for (PrintRowDto printRowDto : printBookingSurcharge) {
                ...
                for (InvoiceProductDto invoiceProductDto : subProductList) {
                    bookingSubTotal = MathUtil.sum(bookingSubTotal, invoiceProductDto.getAmount());
                    bookingVatTotal = MathUtil.sum(bookingVatTotal, invoiceProductDto.getVat());
                    BigDecimal currentTotal = MathUtil.sum(invoiceProductDto.getAmount(), invoiceProductDto.getVat());
